...
class WebsocketCallbacks(BaseModel):
...
class WebsocketInputTransport(BaseInputTransport):
    ...
    def __init__(self, websocket: WebSocket, params: WebsocketParams, callbacks: WebsocketCallbacks, **kwargs):
...
class WebsocketOutputTransport(BaseOutputTransport):
    ...
    def __init__(self, websocket: WebSocket, params: WebsocketParams, **kwargs):
        super().__init__(params, **kwargs)
    ...
    async def set_client_connection(self, websocket: WebSocket | None):
        if self._websocket:
            await self._websocket.close()
    ...
    def write_raw_audio_frames(self, frames: bytes):
        ...
        while len(self._audio_buffer) >= self._params.audio_frame_size:
            ...
            future = asyncio.run_coroutine_threadsafe(
                self._websocket.send_json(answer_dict), self.get_event_loop())
            future.result()
...
class WebsocketTransport(BaseTransport):
    ...
    def __init__(self, websocket: WebSocket, params: WebsocketParams = WebsocketParams(), input_name: str | None = None, output_name: str | None = None, loop: asyncio.AbstractEventLoop | None = None):
        super().__init__(input_name=input_name, output_name=output_name, loop=loop)
        self._params = params

        self._callbacks = WebsocketCallbacks(
            on_client_connected=self._on_client_connected,
            on_client_disconnected=self._on_client_disconnected
        )
        self._input: WebsocketInputTransport | None = None
        self._output: WebsocketOutputTransport | None = None

        self._input = WebsocketInputTransport(websocket, self._params, self._callbacks, name=self._input_name)
        self._output = WebsocketOutputTransport(websocket, self._params, name=self._output_name)
